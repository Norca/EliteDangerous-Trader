...
Sub Process_Globals
	...
	HtmlCSS = "body {background-color: #000000; }" _
		& "table {width: 100%;border: 1px solid #000; text-align: center; }" _
		& "th {font-weight: bold; background-color: #000; color: #ffffff; border-bottom: 1px solid #000; text-align: center; padding: 4px 5px; }" _ 
		& "td {padding: 10px 5px; }" _
		& ".odd {background-color: #006400; } .odd td {border-bottom: 1px solid #006400; }" _
		& ".even {background-color: #2e8b57; } .even td {border-bottom: 1px solid #2e8b57; }" _
	...
	For row = 0 To Limit - 1
		...
		For i = 0 To cur.ColumnCount - 1
			...
			If Clickable Then
				...
				sb.Append(row)
				If cur.GetDouble2(i) > 10000 Then
					sb.Append(".com'>").Append(cur.GetDouble2(i)).Append("</a>")
				Else
					sb.Append(".com'>").Append(cur.GetString2(i)).Append("</a>")
				End If
			Else
				If cur.GetDouble2(i) > 1000 Then
					sb.Append(cur.GetDouble2(i))
				Else
